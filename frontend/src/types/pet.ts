...
/** 被动技能视图（阶段 14：固有自动生效；技能书需启用槽 2/2）。 */
export interface PassiveSkillView {
  ...
  source: string
  signature: boolean
  /** 启用槽位（技能书被动已启用时为 7~8，未启用为 null；固有被动恒为 null）。 */
  slot?: number | null
}
...
export interface BattleSettlement {
  ...
  defeat?: DefeatView
  /** 敌方胜利互动（阶段 12；玩家战败且未逃跑时非空，前端播放）。 */
  victoryInteraction?: VictoryInteractionView
}

/** 敌方胜利互动（阶段 12）：战败表现增强，前端据此播放台词/动作旁白/叫声。 */
export interface VictoryInteractionView {
  id: string
  winnerType: string
  winnerName?: string
  presentationType: 'DIALOGUE' | 'ACTION_NARRATION' | 'CRY'
  actionId?: string
  cry?: string
  /** 命中的战况标签（仅展示用）。 */
  context?: string
