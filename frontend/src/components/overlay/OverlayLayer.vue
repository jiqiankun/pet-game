...
<template>
  ...
  <template v-for="(entry, index) in rendered" :key="entry.id">
    ...
    <OverlayShell
      ...
      :masked="index === rendered.length - 1"
      :z-index="zIndexFor(index)"
      @close="overlayStore.close('QUICK_TEAM')"
    >
      <QuickTeamOverlay />
    </OverlayShell>
    ...
    >
      ...
      <WorldMapView v-if="entry.type === 'WORLD_MAP'" :highlight-region-id="highlightFrom(entry.data)" />
