...
import type { AutoChallengeResult, BossChallengeGroup, BossEncounterSnapshot, BossInfo } from '../types/boss'
...
export const useBossStore = defineStore('boss', () => {
  ...
  const encounterSnapshot = ref<BossEncounterSnapshot | null>(null)
  ...
  /** 读取当前 Boss 难度的固定遭遇；首次挑战前为空。 */
  async function loadEncounterSnapshot(bossId: string, difficulty: string) {
    encounterSnapshot.value = null
    try {
      const res = await apiGet<BossEncounterSnapshot | null>(
        `/api/bosses/${bossId}/encounter-snapshot?difficulty=${encodeURIComponent(difficulty)}`,
      )
      encounterSnapshot.value = res.data ?? null
    } catch (e: any) {
      error.value = e.message ?? '加载 Boss 遭遇失败'
    }
  }

  /** 玩家明确确认后，按当前全局难度重置可重置的遭遇快照。 */
  async function resetEncounterSnapshot(bossId: string, difficulty: string): Promise<boolean> {
    loading.value = true
    error.value = ''
    try {
      const res = await apiPost<BossEncounterSnapshot>(
        `/api/bosses/${bossId}/encounter-snapshot/reset`, { difficulty },
      )
      encounterSnapshot.value = res.data ?? null
      return true
    } catch (e: any) {
      error.value = e.message ?? '重置 Boss 遭遇失败'
      return false
    } finally {
      loading.value = false
    }
