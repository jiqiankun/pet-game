...
- 当前开发阶段：**阶段 6（地图探索与区域系统）— 已完成**
- 已完成阶段：阶段 0（工程脚手架）、阶段 1（配置体系）、阶段 2（存档 + 新游戏 + Bootstrap + 前端首页）、阶段 3（BattleEngine 统一引擎、伤害结算链路、技能/状态/被动配置体系、战斗 REST 接口、前端基础战斗页面）、阶段 4（战斗结算落库、公共经验池、五种升级、等级成长公式、自由加点/洗点、技能解锁装配、宠物详情页、队伍编辑、背包恢复道具、前端养成闭环）、阶段 5（野生遭遇生成、捕捉计算与捕捉/逃跑行动、三档捕捉球、捕获率实时显示、捕捉去向选择、仓库筛选排序/锁定收藏/昵称、放生与临别礼物、27 宠物种族配置补齐）、阶段 6（Phaser 集成与 Tiled 地图管线、起始据点+初始区域+森林区域、刷新组关联、野怪简单行为、HP 持续消耗、营地恢复/传送、区域解锁与移动、地图刷新、采集/宝箱、5 套队伍预设、战败流程、大地图雏形）
- 下一阶段：阶段 7（Boss 系统与重复挑战，以规划文档为准）
- 阶段 6 遗留说明：水域分支/雷属性分支/最终遗迹区域为结构预留（planned: true），阶段 9 开放实际内容；NPC 仅为地图占位对象（阶段 9 对话与任务）；Boss 入口为占位（阶段 7 启用）；隐藏遭遇/随机选择事件/精英个体属阶段 10；简化遭遇入口（`POST /api/wild/battles`）仍保留供开发者模式使用，地图内遭遇通过 `MapExplorationService.validateEncounterGroup` 桥接至同一 WildEncounterService；营地传送触发地图刷新（视为重新进入区域）；地图美术为占位 PNG 资源，验收前不阻塞功能
