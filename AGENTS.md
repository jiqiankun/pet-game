...
1. **先读文档**：阅读需求文档、技术方案、分阶段开发规划与本文件。
...
7. **验收自查**：完成后逐条对照该阶段「完成标准」与「核心业务规则」验证，并汇报：已完成项、验证结果、遗留问题、风险。
8. **更新进度**：阶段验收通过后，更新本文件「当前阶段状态」一节。

**阶段验收通过前，不开启下一阶段开发。**

## 6. 当前阶段状态

- 当前开发阶段：**阶段 0（工程脚手架与构建部署流水线）— 未开始**
- 已完成阶段：无
...
- **配置驱动**：核心数值（倍率、概率、成长、冷却、奖励等）一律配置化，不硬编码；新增宠物/技能/Boss/地图正常情况下只加配置与资源、不改业务代码；所有配置接入启动校验（严重错误启动失败），支持外部配置目录同 ID 覆盖，不做热更新。
- **统一随机**：所有随机场景（暴击、命中、捕获、掉落、资质、稀有技能、特殊外观、放生礼物）走统一随机入口并支持固定种子，禁止业务代码直接使用原生随机 API。
- **事务**：战斗结算、捕获、放生、幸运兑换、存档导入等关键流程必须事务化，不允许部分成功。
- **事件化战斗表现**：后端输出标准战斗事件，前端按事件播放表现；不返回表现层指令。
- **数据预留**：仅保留文档明确要求的预留项（saveId、成长阶段/阶段等级上限/进化相关字段、4 上场底层支持），不为假设需求加冗余字段。
- **版本号**：gameVersion（发布版本）、saveVersion（存档结构）、configVersion（配置结构）职责分离，随变更维护。
- **日志**：记录启动、配置校验错误、数据库异常、存档导入导出、开发者操作；高频战斗数值不写 INFO 日志。
- **资源命名**：资源路径统一使用 ID（如 `pets/fire/PET_FIRE_001.png`），禁止中文文件名。
- **测试重点**：战斗引擎（顺序/克制/本属性/暴击区间/冷却/Buff/换宠/被动）、捕捉计算（HP/异常/球倍率/边界）、成长（升级/点数/上限）、Boss（3/6/10 解锁、幸运值累计与溢出）、放生（50% 加成上限）。不追求高覆盖率，Phaser 表现以人工测试为主。
...
正确性 > 可用性 > 一致性 > 美观程度。前期不为视觉效果阻塞核心功能；美术资源可用占位物先行，验收前必须列出待补清单。响应式要求：PC 优先，兼容平板与手机，核心玩法全部可用。
