...
- 当前开发阶段：**阶段 5（捕捉与宠物仓库管理）— 进行中**
- 已完成阶段：阶段 0（工程脚手架）、阶段 1（配置体系）、阶段 2（存档 + 新游戏 + Bootstrap + 前端首页）、阶段 3（BattleEngine 统一引擎、伤害结算链路、技能/状态/被动配置体系、战斗 REST 接口、前端基础战斗页面）、阶段 4（战斗结算落库、公共经验池、五种升级、等级成长公式、自由加点/洗点、技能解锁装配、宠物详情页、队伍编辑、背包恢复道具、前端养成闭环）
- 下一阶段：阶段 6（地图探索与区域系统，以规划文档为准）
- 阶段 4 遗留说明：面板属性公式已从阶段 3 临时实现正式化为 PetGrowthService（资质影响升级成长而非即时加属性）；通用种族配置目录 `game-config/pets/` 为空，留待阶段 5 捕捉系统补齐
- 需求模糊点：已裁决完成，见规划文档 §9.3（决策一~七 + 决策八：恢复道具仅战斗外使用、战斗内无道具行动）；数值均配置化，规则结构不得自行变更
