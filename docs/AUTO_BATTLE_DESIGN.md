...
阶段 3/4 曾裁决「恢复道具仅战斗外使用，战斗内无道具行动」。本阶段提示词明确要求自动战斗支持战斗内自动恢复/复苏道具（默认关闭、显式开启才生效）。按文档优先级（用户当前明确要求 > 历史结论），本阶段实现战斗内 ITEM 行动，但**手动战斗仍不提供道具行动入口**（仅自动 AI 在开关开启时使用），保持手动体验与既有裁决一致。
