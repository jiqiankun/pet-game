...
- 当前开发阶段：**阶段 3（战斗引擎核心）— 已完成**
- 已完成阶段：阶段 0（工程脚手架）、阶段 1（配置体系）、阶段 2（存档 + 新游戏 + Bootstrap + 前端首页）、阶段 3（BattleEngine 统一引擎、伤害结算链路、技能/状态/被动配置体系、战斗 REST 接口、前端基础战斗页面）
- 下一阶段：阶段 4（以规划文档为准）
- 阶段 3 遗留说明：面板属性公式（等级成长 + 资质 + 自由点数）为 BattleService 阶段 3 临时实现，待培养系统阶段正式化；阶段 2 遗留的 created_at/updated_at 自动填充缺陷已随阶段 3 修复（MybatisMetaConfig）
