...
> 定位：**扫描现状 → 找缺失 → 定用途 → 去重 → 出清单 → 供后续 AI 批量生成接入**。本阶段不生成任何图片、不修改任何页面。
...
| 技能 | 66（主动 61 + 被动 5；稀有主动 15） | `skills.yml` |
| 状态 | 24 | `statuses.yml` |
| 道具 | 34（恢复 4 + 捕捉 3 + 净化 1 + 材料 14 + 技能书 12） | `items.yml` |
...
- 主动技能 61（含 15 稀有）+ 被动技能 5 = **66**。
- 被动技能（不屈/顺风/余烬/再生/厚皮）可复用强化/火焰/回血类图标，不单独设计。
...
**建议 VFX 数量**：约 **30~40 个模板**（9 元素 × 2~3 档 + 治疗/护盾/强化/削弱/控制/中毒/特殊 ~10），即可覆盖全部 66 技能，避免无限膨胀。
...
| 技能书（12 本） | 12 | 书底图 + 技能属性图标 | 1 书底图（复用技能图标） |

**合计：约 30 款**（技能书 12→1 底图、结晶 9→复用属性色、核心 5→模板）。
