...
- 需求模糊点：已裁决完成，见规划文档 §9.3（决策一~七 + 决策八：恢复道具仅战斗外使用、战斗内无道具行动）；阶段 5 补充裁决：捕捉球新游戏赠送（10/5/2）、逃跑必定成功同战败结算、野生奖励按遭遇组配置（被捕捉宠物不参与）、初始宠物等级 5 级（防初始打不过野生宠物，HP/技能/面板按 Lv.5 计算）、新游戏赠送小型恢复药 ×10 与复苏药 ×2；数值均配置化，规则结构不得自行变更
